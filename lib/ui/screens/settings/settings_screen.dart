...
class _IntegrationsSectionState extends ConsumerState<_IntegrationsSection> {
  ...
  void _showInstacartOptions(BuildContext context) {
    ...
    showModalBottomSheet(context: context, builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ListTile(leading: const Icon(Icons.vpn_key_outlined), title: Text(l10n.setCustomApiKey), subtitle: Text(l10n.useOwnInstacartKey), onTap: () {
        Navigator.pop(ctx);
        _showApiKeyDialog(context, provider: GroceryProvider.instacart, title: l10n.instacartApiKey, hint: 'Bearer key from Developer Dashboard');
      }),
      ListTile(leading: Icon(Icons.link_off, color: theme.colorScheme.error), title: Text(l10n.resetToDefaultKey, style: TextStyle(color: theme.colorScheme.error)), subtitle: Text(l10n.removeCustomKey), onTap: () async {
        Navigator.pop(ctx); await GroceryService.disconnect(GroceryProvider.instacart); _checkStoreStatus();
      }),
      const SizedBox(height: 16),
    ])));
  }
  ...
  void _showApiKeyDialog(BuildContext context, {required GroceryProvider provider, required String title, required String hint}) {
    final l10n = AppLocalizations.of(context)!;
    final controller = TextEditingController();
    showDialog(context: context, builder: (ctx) => AlertDialog(
      title: Text(title),
      content: TextField(controller: controller, decoration: InputDecoration(hintText: hint, border: const OutlineInputBorder()), autofocus: true),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: Text(l10n.actionCancel)),
        FilledButton(onPressed: () async {
          final key = controller.text.trim(); if (key.isEmpty) return;
          Navigator.pop(ctx); await GroceryService.configureInstacart(apiKey: key); _checkStoreStatus();
          if (mounted) AppSnackbar.success(context, l10n.apiKeySaved);
